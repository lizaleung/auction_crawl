...
from auction_crawl.items import BonhamsCalendarItem, BonhamsCalendarDetailItem


class BonhamscalendarSpider(scrapy.Spider):
    name = 'bonhamsCalendar'
    allowed_domains = ['bonhams.com']
    start_urls = ['http://bonhams.com/']
    folder_name = "bonhams/calendar"

    def start_requests(self):
        self.url = "https://www.bonhams.com/api/v1/search_json/?content=sale&date_range=future&exclude_sale_type=3&length=100&page=1&randomise=False&page={page}"
    ...
    def parse(self, response, *args, **kwargs):
        ...
        for item in items:
            ...
            _item = BonhamsCalendarItem(url=url, auction_id=auction_id, title_txt=title_txt, location_txt=location_txt,
            ...
            yield Request("https://www.bonhams.com/auctions/{}/".format(auction_id), self.parse_detail_info,
                          meta={"item": dict(_item)})
            self.lot_page_url = "https://api01.bonhams.com/api/search/auction/{auction_id}/?page={page}&page_size=100"
            yield Request(self.lot_page_url.format(auction_id=auction_id, page=1), self.parse_detail_list,
    ...
    def parse_detail_info(self, response):
        ...
        try:
            ...
            e_catelogue = None
        catelogue = catelogue or e_catelogue or None
        item["catelogue"] = catelogue if catelogue != "/catalogues/" else None
        yield BonhamsCalendarItem(**item)
    ...
    def parse_detail_list(self, response):
        ...
        if page and page == 1 and num_page > 1:
            for i in range(2, num_page):
                yield Request(self.lot_page_url.format(auction_id=auction_id, page=i), self.parse_detail_list,
                              meta={"start_date": start_date, "end_date": end_date, "url": response.meta["url"],
        ...
        for item in data["results"]:
            ...
            yield BonhamsCalendarDetailItem(url=url, auction_id=auction_id, lot_no=lot_no, title_txt=title_txt,
